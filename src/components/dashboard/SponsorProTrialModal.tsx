...
const FEATURES = [
  'Sponsee dashboard with vitals and alerts',
  '14-day mood trends with full history',
  'Step work review and feedback tools',
  'Meeting attendance reports',
  'Private sponsor notes and reminders',
]

export default function SponsorProTrialModal({ userId, onClose, onTrialStarted }: Props) {
    ...
    >
      <div style={{ background: '#fff', borderRadius: 20, width: '100%', maxWidth: 480, padding: '36px 32px', boxShadow: '0 24px 64px rgba(0,51,102,0.18)', position: 'relative', maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 26, fontWeight: 600, color: 'var(--navy)', letterSpacing: '-0.5px', margin: '0 0 8px', textAlign: 'center' }}>
          Try Sponsor Pro free for 30 days
        </h2>
        <p style={{ fontSize: 14, color: 'var(--mid)', lineHeight: 1.6, margin: '0 0 24px', textAlign: 'center' }}>
          Powerful tools to help you be the best sponsor you can be — mood tracking, step work review, meeting insights, and more.
        </p>
        ...
        {error && (
          <div style={{ background: '#FEE', border: '1px solid #F5C6CB', borderRadius: 8, padding: '9px 13px', fontSize: 13, color: '#721C24', marginBottom: 14 }}>
            {error}
          </div>
        )}
        ...
        <button
          ...
          disabled={loading}
          style={{ width: '100%', background: 'var(--teal)', color: '#fff', border: 'none', borderRadius: 10, padding: '14px', fontSize: 15, fontWeight: 600, cursor: loading ? 'not-allowed' : 'pointer', opacity: loading ? 0.7 : 1, fontFamily: 'var(--font-body)', marginBottom: 10, letterSpacing: '-0.2px' }}>
        ...
        {/* Mission box */}
        <div style={{ borderLeft: '3px solid var(--teal)', paddingLeft: 16, background: 'rgba(42,138,153,0.04)', borderRadius: '0 8px 8px 0', padding: '14px 16px 14px 16px' }}>
