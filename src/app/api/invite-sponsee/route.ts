...
export async function POST(req: NextRequest) {
  ...
  // Parse body
  let to: string, subject: string, body: string, sponsorName: string
  try {
    ({ to, subject, body, sponsorName } = await req.json())
  } catch {
  ...
  // Rate limit: max 5 invites per sponsor per 24 hours
  ...
  if (sendError) {
    console.error('[invite-sponsee] Resend error:', sendError)
    return NextResponse.json({ error: 'Failed to send email. Please try again.' }, { status: 500 })
  }

  // Record the invite
  await admin.from('sponsee_invites').insert({
    sponsor_id: user.id,
